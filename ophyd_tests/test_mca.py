...
from ophyd import EpicsDXP, EpicsMCA, EpicsSignalRO
...
def mca_test_ioc(request):
    name = "MCA test IOC"
    pvs = dict(mca_prefix=f"{MCA_PREFIX}mca", dxp_prefix=f"{MCA_PREFIX}dxp:")

    sig = EpicsSignalRO(pvs["mca_prefix"], name="check")
    try:
    ...
    return SimpleNamespace(
        process=None, prefix=MCA_PREFIX, name=name, pvs=pvs, type="caproto"
    )
